...
| # | Finding | Severity | Status | Evidence |
| --- | --- | --- | --- | --- |
| 2.1 | **The review loop's clean verdict can be manufactured.** `acceptedByDesign` is populated from the AUTHOR model's own verdicts, then unconditionally deleted from the reviewer's later findings *before* the clean check and *before* the oscillation guard. A model that declares a finding invalid ships with the reviewer still objecting. | **CRITICAL** | OPEN | Drove the real pipeline: reviewer raises the same MATERIAL finding in rounds 1 and 2, author says `valid:false` → `outcome: shipped, unresolved: 0, fixApplied: false`. Control with `valid:true` → `refused-review`. Falsifies three comments in the same file and the project's headline claim. |
