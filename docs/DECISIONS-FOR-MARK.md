...
## 18. `BO7A`'s own gate (CLI's A1 on `origin/scoring`) has passed, but `A1` is not merged into `codex-lane` — writing catalogue entries now would still be "written wrong," just against a different stale schema than the checklist's own warning names — RESOLVED, 2026-09-15 (this merge)

**Renumbered from #12, then #14, during the `origin/main` <- `codex-lane` merge, 2026-09-15:** both branches independently numbered new entries starting at the same next integer again — the exact divergence this file's own header (§"HOW IT IS USED") already documents as structural, not carelessness. CLI's own #14–#17 (this file's own `unitQuality`, `medianWealth`, background-promotion rule, and T1–T3 ownership decisions, immediately above) are the numbers CLI's own already-pushed history may reference; this decision (and #19 below) moved instead, since neither had been cited by number outside this file yet. `cbf3911` and `67b2a41` (earlier `codex-lane` commits) say "Decision 12"; commit `f3d7296` (this run) says "docs/DECISIONS-FOR-MARK.md #15" for the entry now below at #19 — both are now stale references to this same file's own content, not to different decisions.
...
## 19. BO7A's own `street-cross` binding is very likely wrong — verified by rendering, not by trusting the filename
