...
## 14. `BO7A`'s own gate (CLI's A1 on `origin/scoring`) has passed, but `A1` is not merged into `codex-lane` — writing catalogue entries now would still be "written wrong," just against a different stale schema than the checklist's own warning names

**Renumbered from #12 during the `origin/main` <- `codex-lane` merge, 2026-09-15:** both lanes independently numbered a new entry "## 12" on their own branch. CLI's own #12/#13 (this file's own catalogue-migration and falloff decisions, immediately above) are cross-referenced BY NUMBER from inside CLI's own already-pushed commit message (`84d4a16`: "per DECISIONS-FOR-MARK #12 point 3"); this decision was not yet cited by number anywhere outside this file, so it moved instead. `cbf3911` and `67b2a41` (this branch's own already-pushed commits) still say "Decision 12" — that number is now stale; this is the same decision, renumbered on merge, not a new one.
