...
function tokenize(text) {
  ...
  for (const t of rawTokens) {
    ...
    if (t === "skyscraper") {
      expanded.add("sky");
      expanded.add("scraper");
      expanded.add("tower");
      expanded.add("highrise");
    }
    if (t === "highrise" || t === "high-rise") {
      expanded.add("high");
      expanded.add("rise");
      expanded.add("tower");
      expanded.add("skyscraper");
    }
    if (t === "waterfront") {
      expanded.add("water");
      expanded.add("front");
...
function generateCorpus(registry) {
  const corpus = [];
...
var CACHED_CORPUS = null;
var CACHED_REGISTRY_REF = null;
function getOrBuildCorpus(registry) {
...
function inferCategoryAffinity(queryRaw) {
  const q = queryRaw.toLowerCase();
  if (q.includes("tower") || q.includes("skyscraper") || q.includes("high-rise") || q.includes("highrise") || q.includes("house") || q.includes("home") || q.includes("villa") || q.includes("chalet") || q.includes("bungalow") || q.includes("townhouse") || q.includes("apartment") || q.includes("ranch") || q.includes("mansion") || q.includes("warehouse") || q.includes("shop") || q.includes("bodega") || q.includes("office") || q.includes("flat") || q.includes("penthouse") || q.includes("building")) {
    return "buildings";
  }
...
function scoreDocument(queryTokens, docTokens, docText, queryRaw, docCategory) {
  if (queryTokens.length === 0 || docTokens.length === 0) return 0;
  const docTokenFreq = /* @__PURE__ */ new Map();
  ...
  if (inferredCat) {
    if (docCategory === inferredCat) score += 8;
    else score -= 15;
  }
...
function rerankCandidates(query, candidates) {
  ...
  return candidates.map((cand) => {
    ...
    if (qLower.includes("small") || qLower.includes("low rise") || qLower.includes("cottage")) {
      if ((cand.entry.levels || 1) <= 2) boost += 3;
    }
...
function findModels(description, options = {}) {
  const {
    registry = {},
    fits,
    standsOn,
