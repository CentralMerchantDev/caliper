...
import { Bm25Index, computeNDCGAtK, type SciFactDoc } from "./beirSciFactSubset.ts";
...
test("full ArguAna BM25 is rejected because current self-exclusion does not reproduce the paper-era result", () => {
  ...
  const measured = queries.reduce((sum, query) => {
    const scoredRanking = index.rank(query.text).filter((documentId) => documentId !== query.id);
...
test("ArguAna scoring excludes the query document from raw candidates", () => {
  ...
  const scoredRanking = rawCandidates.filter((documentId) => documentId !== query.id);
